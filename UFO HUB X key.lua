...
pcall(function()
    ...
    if old and old:IsA("ScreenGui") then
        _G.__UFO_SOFT_PARENT(old) -- ไม่ลบของเดิม แค่ย้ายให้ปลอดภัย
    end
end)
...
-- - ปุ่ม Get Key คัดลอกลิงก์พร้อม uid/place (รองรับ auto-fill)
...
btnGetKey.MouseButton1Click:Connect(function()
    if submitting then return end
    submitting = true
    local oldText = btnGetKey.Text
    btnGetKey.Text = "⏳ Getting key..."
    ...
    if ok and data and data.ok then
        local got = (data.key and tostring(data.key) ~= "" and tostring(data.key)) or nil
        if got then
            keyBox.Text = got
            refreshSubmit()
            showToast("🔑 ได้คีย์จากเซิร์ฟเวอร์แล้ว", true)
            setStatus("ระบบใส่คีย์ให้แล้ว กด Submit ได้เลย", true)
            btnGetKey.Text = "✅ Key filled"
        else
            ...
            setClipboard(link)
            showToast("คัดลอกลิงก์แล้ว เปิดในเบราว์เซอร์เพื่อรับคีย์", false)
            setStatus("เซิร์ฟเวอร์ยังไม่คืนค่า key ให้ กดเปิดลิงก์เองได้", nil)
        ...
        setClipboard(link)
        showToast("เชื่อมต่อเซิร์ฟเวอร์ไม่ได้ คัดลอกลิงก์ให้แล้ว", false)
        setStatus("เปิดลิงก์ในเบราว์เซอร์เพื่อรับคีย์ แล้วนำมาวาง", false)
        btnGetKey.Text = "⚠️ Link copied"
    end

    task.delay(1.6, function()
        btnGetKey.Text = oldText
        submitting = false
    end)
end)
